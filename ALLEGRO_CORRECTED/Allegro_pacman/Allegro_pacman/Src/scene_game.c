...
static void init(void) {
	...
	game_main_Score = 0;
	// create map
	basic_map = create_map("C:/CS/ALLEGRO_CORRECTED/Allegro_pacman/Allegro_pacman/map_nthu.txt");
...
static void status_update(void) {
	...
	if (pman->powerUp)
	{
		int timer = get_power_up_timer_tick();
		int duration = get_power_up_duration();
		//printf("DURATION: %d\n", duration - timer);
		if (duration - timer > 0) 
		{
			for (int i = 0; i < 4; i++) {
	...
	for (int i = 0; i < GHOST_NUM; i++) {
		...
		else if (ghosts[i]->status == FREEDOM)
		{
			...
			const RecArea pacman_coll = getDrawArea((object*)pman, GAME_TICK_CD);
			const RecArea ghost_coll = getDrawArea((object*)ghosts[i], GAME_TICK_CD);
			
			if (!cheat_mode && RecAreaOverlap(&pacman_coll, &ghost_coll))
			{
...
static void draw(void) {
	...
	char scoreboard[20];
	int result = sprintf(scoreboard, "SCORE: %d", game_main_Score);

	if (result) {
		al_draw_text(menuFont, al_map_rgb(0, 255, 0), 10, 10, ALLEGRO_ALIGN_LEFT, scoreboard);
	}
...
static void on_key_down(int key_code) {
	switch (key_code)
	{
		// TODO-HACKATHON 1-1: Use allegro pre-defined enum ALLEGRO_KEY_<KEYNAME> to control pacman movement
...
int64_t get_power_up_timer_tick(void) {
	int power = al_get_timer_count(power_up_timer);
	return power;
}
...
static void game_over_screen() {
	...
	if (result) {
		al_draw_text(menuFont, al_map_rgb(0, 255, 0), 400, 450, ALLEGRO_ALIGN_CENTER, scoreboard);
	}
...
static void victory_screen() {
	...
	if (result) {
		al_draw_text(menuFont, al_map_rgb(0, 255, 0), 400, 400, ALLEGRO_ALIGN_CENTER, scoreboard);
	}
