...
static bool pacman_movable(const Pacman* pacman, const Map* M, Directions targetDirec) {
	...
	int validx = pacman->objData.Coord.x, validy = pacman->objData.Coord.y;
	...
	if (is_wall_block(M, validx, validy) || is_room_block(M, validx, validy))
		return false;
...
void pacman_draw(Pacman* pman) {
	...
	if (!game_over) {
		...
		else if(pman->objData.moveCD % 4 != 0){
			offset = 16;
		}
		...
		*/
		...
		while (true) {
			...
			if (timer <= 12) {
				al_draw_scaled_bitmap(pman->die_sprite, 16*timer, 0,
					16, 16,
					drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
					draw_region, draw_region, 0
				);
