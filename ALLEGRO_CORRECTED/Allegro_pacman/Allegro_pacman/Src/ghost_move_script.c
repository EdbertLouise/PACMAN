...
static void ghost_move_script_FREEDOM_random(Ghost* ghost, Map* M) {
	...
	int rand = generateRandomNumber(0, cnt);
	ghost_NextMove(ghost, proba[rand]);
	*/
	...
	//printf("CTRRR: %d\n", counter_one);
	for (Directions i = 1; i <= 4; i++) {
		if (i != counter_one && ghost_movable(ghost, M, i, ghost->status == FREEDOM)) {
			proba[cnt++] = i;
		}
	}
	//printf("CNT: %d\n", cnt);
...
static void ghost_move_script_FLEE(Ghost* ghost, Map* M, const Pacman * const pacman) {
	...
	if (shortestDirection == UP) {
		for (Directions i = 1; i <= 4; i++)
			if (ghost_movable(ghost, M, i, ghost->status == FREEDOM) && i != 1) proba[cnt++] = i;
		int rand = generateRandomNumber(0, cnt);
		ghost_NextMove(ghost, proba[rand]);
	}
	else if (shortestDirection == LEFT) {
		for (Directions i = 1; i <= 4; i++)
			if (ghost_movable(ghost, M, i, ghost->status == FREEDOM) && i != 2) proba[cnt++] = i;
		int rand = generateRandomNumber(0, cnt);
		ghost_NextMove(ghost, proba[rand]);
	}
	else if (shortestDirection == RIGHT) {
		for (Directions i = 1; i <= 4; i++)
			if (ghost_movable(ghost, M, i, ghost->status == FREEDOM) && i != 3) proba[cnt++] = i;
		int rand = generateRandomNumber(0, cnt);
		ghost_NextMove(ghost, proba[rand]);
	}
	else if (shortestDirection == DOWN) {
		for (Directions i = 1; i <= 4; i++)
			if (ghost_movable(ghost, M, i, ghost->status == FREEDOM) && i != 4) proba[cnt++] = i;
		int rand = generateRandomNumber(0, cnt);
		ghost_NextMove(ghost, proba[rand]);
	}
...
void ghost_move_script_random(Ghost* ghost, Map* M, Pacman* pacman) {
	...
	if (!movetime(ghost->speed))
		...
		case BLOCKED:
			...
			if ((al_get_timer_count(game_tick_timer) > GO_OUT_TIME && pacman->powerUp == false) || win_con == true)
				ghost->status = GO_OUT;
			break;
		case FREEDOM:
...
void ghost_move_script_shortest_path(Ghost* ghost, Map* M, Pacman* pacman) {
	...
	if (!movetime(ghost->speed))
		...
		case BLOCKED:
			ghost_move_script_BLOCKED(ghost, M);
			if (al_get_timer_count(game_tick_timer) - ghost->go_in_time > GO_OUT_TIME && pacman->powerUp == false)
				ghost->status = GO_OUT;
			break;
		case FREEDOM:
			//printf("%d\n", rand);
