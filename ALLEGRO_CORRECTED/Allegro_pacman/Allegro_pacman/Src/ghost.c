...
void ghost_draw(Ghost* ghost) {
	...
	else if (ghost->objData.moveCD % 4 != 0) {
		bitmap_x_offset = 16;
	}
	if (ghost->status == FLEE) {
		...
		*/ 
		...
		int time = get_power_up_timer_tick();

			if (duration * 7 / 10 - time <= 0) 
			{
		...
		case RIGHT:
			...
			break;
		case DOWN:
			al_draw_scaled_bitmap(ghost->dead_sprite, 64, 0,
				16, 16,
				drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
				draw_region, draw_region, 0
			);
...
bool ghost_movable(const Ghost* ghost, const Map* M, Directions targetDirec, bool room) {
	...
	int validX = ghost->objData.Coord.x, validY = ghost->objData.Coord.y;
	...
	if (is_wall_block(M, validX, validY) || (room && is_room_block(M, validX, validY)))
		return false;
